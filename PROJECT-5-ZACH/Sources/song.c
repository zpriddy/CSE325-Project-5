...
1. Provides a structure type to store the information (title and notes) for a song. For example,
typedef struct {
 note notes[SONG_MAX_NOTES];
 int num_notes;
 int tempo;
 char title[SONG_MAX_TITLE];
} song_t;
2. Provides functionality to create a song_t variable given the title and notes.
3. Provides accessor and mutator functions to permit access to the song_t data members.
 */
